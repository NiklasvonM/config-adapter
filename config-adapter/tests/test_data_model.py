from config_adapter.model_generation.strategies.util.model import DataModel, UnionType


def test_model_creation():
    model = DataModel(name="TestModel", predefined=True)
...
def test_add_field_to_model():
    model = DataModel(name="TestModel")
...
def test_union_type_representation():
    ...
    assert str(union) in {"str | int", "int | str"}


def test_model_product_creates_combined_fields():
    model1 = DataModel(name="User")
    model1.add_field("name", "str")
    model2 = DataModel(name="Admin")
